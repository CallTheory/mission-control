    ...
    @if($boardChecks->count())
        ...
        <table class="min-w-full divide-y divide-border-soft text-left">
            ...
            <tr class="sticky top-0">
                <th scope="col" class="px-6 py-3 text-xs font-medium text-muted 0 uppercase tracking-wider">msgId</th>
                <th scope="col" class="px-6 py-3 text-xs font-medium text-muted 0 uppercase tracking-wider">isCallId</th>
                <th scope="col" class="px-6 py-3 text-xs font-medium text-muted 0 uppercase tracking-wider">Status</th>
                <th scope="col" class="px-6 py-3 text-xs font-medium text-muted 0 uppercase tracking-wider">Actions</th>
                <th scope="col" class="px-6 py-3 text-xs font-medium text-muted 0 uppercase tracking-wider">Category</th>
                <th scope="col" class="px-6 py-3 text-xs font-medium text-muted 0 uppercase tracking-wider">Comments</th>
            </tr>
            </thead>
            <tbody class="bg-surface divide-y divide-border-soft ">
            @foreach($boardChecks as $row)

                <tr class="group transform transition duration-700 ease-in-out text-surface-fg-soft">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap font-semibold text-sm transform transition duration-700 ease-in-out">
                        <a class="flex text-primary hover:text-primary" target="_blank" href="/utilities/call-lookup/{{ $row->callId }}">
                    ...
                    <td>
                        @if(is_null($row->marked_ok_at) && is_null($row->problem_verified_at))
                            @if(!is_null($row->approved_at))
                                <span class="text-xs px-2 py-1 rounded shadow bg-success text-success">Dispatcher Approved</span>
                            @elseif(!is_null($row->problem_found_at))
                                <span class="text-xs px-2 py-1 rounded shadow bg-warning text-warning">Issue Flagged</span>
                            @else
                                <span class="text-xs px-2 py-1 rounded shadow bg-warning text-warning">Unknown Issue</span>
                            @endif

                        @elseif($row->marked_ok_at)
                            <span class="text-xs px-2 py-1 rounded shadow bg-success text-success">Supervisor OK</span>
                        @elseif($row->problem_verified_at)
                            <span class="text-xs px-2 py-1 rounded shadow bg-danger text-danger">Problem Verified</span>
                        @else
