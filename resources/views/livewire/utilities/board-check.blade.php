            ...
            <tbody class="bg-surface divide-y divide-border-soft ">
                @foreach($boardChecks as $row)
                    <tr class="group  transform transition duration-700 ease-in-out  ">
                        ...
                        <td>
                            <span class="text-xs px-2 py-1 rounded shadow bg-warning-soft border border-warning text-warning">Needs Reviewed</span>
                        </td>
                        ...
                        <td class="flex my-2">
                            ...
                            <span class="inline-flex shadow rounded-md">
                                <a wire:click="$dispatch('openModal',{ component: 'utilities.board-dispatcher-review-message', arguments: {{ json_encode(['msgId' => $row->msgId, 'isCallID' => $row->callId]) }}  })"
                                    ...
                                    class="cursor-pointer -ml-px relative inline-flex items-center px-2 py-1 rounded-md border border-border bg-surface text-sm text-surface-fg-soft hover:bg-surface-2 focus:z-10 focus:outline-hidden focus:ring-1 focus:ring-primary focus:border-primary">
