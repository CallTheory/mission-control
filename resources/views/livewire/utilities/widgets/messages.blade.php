    ...
    @foreach($messages as $message)
        ...
        @if(!is_null($message->msgId))
            ...
            <div x-data="{ currentMessageRevision: -1 }">
                <div class="flex px-2 py-2 my-2 align-middle border-b border-border">
                    <svg class="w-5 h-5 mx-0 inline-flex text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                    <strong>Message <code class="font-normal ml-2 border rounded px-1 py-0.5 bg-surface-2 border-border text-xs">msgId&middot;{{ $message->msgId }}</code></strong>

                    @if(count($revisions) > 0)
                        <select x-model="currentMessageRevision" class="border rounded border-border shadow text-xs my-0 ml-2">
                ...
                <div x-show="currentMessageRevision == -1" class="block text-surface-fg rounded px-2 py-2">

                    @if(strlen(trim($message->Summary)) > 0)
                        <code class="block p-4 border rounded shadow bg-surface-2 border-border text-sans break-words">{!! Helpers::formatMessageSummary($message->Summary) !!}</code>
                    @else
                        <code class="block p-4 border rounded shadow bg-surface-2 border-border text-sans break-words">{{ $message->Index }}</code>
                    @endif
                    ...
                    <div class="flex flex-wrap mt-4">
                        ...
                        @if($message->Discarded)
                            <div class="flex rounded text-subtle bg-surface-inverse px-2 py-1 text-xs my-1 mr-2 whitespace-nowrap">
                        ...
                        @endif
                        @if($message->Archived)
                            <div class="flex rounded text-accent-2-fg bg-accent-2 px-2 py-1 text-xs my-1 mr-2 whitespace-nowrap" >
                        ...
                        @endif
                        @if($message->Special)
                            <div class="flex rounded px-2 py-1 text-accent-2-soft-fg bg-accent-2-soft text-xs my-1 mr-2 whitespace-nowrap">
                        ...
                        @if($message->copiedFrom)
                            <div class="flex rounded text-surface-fg-soft bg-surface-3 px-2 py-1 text-xs my-1 mr-2 whitespace-nowrap">
                        ...
                        @if($messageKeywords)
                            @foreach($messageKeywords as $kws)
                                @if($kws->msgId === $message->msgId)
                                    <div class="flex rounded text-surface-fg bg-surface border border-border px-2 py-1 text-xs my-1 mr-2 whitespace-nowrap">
                ...
                @foreach($revisions as $order => $revision)
                    <div x-show="currentMessageRevision == {{ $order }}" class="block text-surface-fg rounded px-2 py-2 mx-2">

                        <div x-data="{ currentTab: 1}">
                            <!-- Tabs -->
                            <ul class="flex space-x-2">
                                <li class="mr-2">
                                    <h3 class="text-sm font-semibold text-primary text-left mb-2">Viewing &rarr; Revision {{ $order+1 }}</h3>
                                </li>
                                <li  @click="currentTab = 1"
                                     class="px-2"
                                     :class="{ 'border-b border-primary': currentTab === 1  }">
                                    <button href="#">Summary</button>
                                </li>
                                @if(strlen(trim($revision['diff'])) > 0)
                                    <li @click="currentTab = 2"
                                        class="px-2"
                                        :class="{ 'border-b border-primary': currentTab === 2  }">
                                ...
                                @if(count($revision['annotations']) > 0)
                                    <li @click="currentTab = 3"
                                        class="px-2"
                                        :class="{ 'border-b border-primary': currentTab === 3  }">
                                        <button href="#">Annotations</button>
                                    </li>
                                @endif
                                <li @click="currentTab = 4"
                                    class="px-2"
                                    :class="{ 'border-b border-primary': currentTab === 4  }">
                            ...
                            <div class="my-4">
                                <div x-show="currentTab === 1">
                                    <code class="block p-4 border rounded shadow bg-surface-2 text-sans break-words">{!! Helpers::formatMessageSummary($revision['summary']) !!}</code>
                                </div>
                                <div x-show="currentTab === 2">
                                    @if(strlen(trim($revision['diff'])) > 0)
                                        ...
                                        {!! $revision['diff3'] !!}
                                    @else
                                        <div class="my-4 block text-muted">No diff generated for this revision</div>
                                    @endif
                                ...
                                <div x-show="currentTab === 3">
                                    ...
                                    @empty
                                        <div class="my-2 block text-xs text-muted">No annotations</div>
                                    @endforelse
                                </div>
                                <div x-show="currentTab === 4">
                                    <table class="text-sm py-4 my-4 table-auto border divider divide-border border-border w-full bg-surface-2">
                                        <tbody class="my-4 px-4 border rounded">
                                            @foreach($revision['fields'] as $fieldkey => $fieldvalue)
                                                <tr>
                                                    <th class="py-1 text-right {{ is_array($fieldvalue) ? 'text-muted ' : 'text-surface-fg' }}">&nbsp;{{ $fieldkey }}</th>
                                                    <td class="px-2 {{ is_array($fieldvalue) ? 'text-subtle ' : 'text-primary' }}">&rarr;</td>
