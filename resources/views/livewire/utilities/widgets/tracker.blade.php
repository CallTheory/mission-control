            ...
            @foreach($tracker as $t)
                @if(is_object($t))
                    <div class=" bg-white   group
                        ...
                        <div class="table-cell px-6 py-1 text-xs break-words font-medium text-gray-900  border-b ">
                            {{ Carbon::parse($t->Stamp, $timezone)->timezone(Auth::user()->timezone)->format("m/d/Y g:i:s.u A T")  }}
                        </div>
                        <div class="table-cell px-6 py-1 whitespace-nowrap text-sm font-normal text-gray-900  border-b ">
                            {{ $t->CallId }}
                        </div>
                        <div class="table-cell px-6 py-1 whitespace-nowrap text-sm font-medium text-gray-900  border-b ">
                            {{ $t->clientNumber }}
                        </div>
                        <div class="table-cell px-6 py-1 whitespace-nowrap text-sm font-medium text-gray-900  border-b ">
                            {{ $ck[$t->callType] ?? $t->callType }}
                        </div>
                        <div class="table-cell px-6 py-1 whitespace-nowrap text-sm font-thin text-gray-900  border-b ">
                            {{ $tt[$t->type] ?? $t->type }}
                        </div>
                        <div class="table-cell px-6 py-1 whitespace-nowrap text-sm font-thin text-gray-900  border-b ">
                            {{ $cs[$t->callState] ?? $t->callState }}
                        </div>
                        <div class="table-cell px-6 py-1 whitespace-nowrap text-xs font-medium text-gray-900  border-b ">
                            @if($t->Name)
                        ...
                        <div class="table-cell px-6 py-1 wrap text-xs break-words font-normal text-gray-900  border-b ">
                            {{ $t->value }}
                        </div>
                    </div>
                @endif
