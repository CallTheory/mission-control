                ...
                <div class="space-x-5 flex">
                    <div class="relative flex items-start">
                        ...
                        <div class="ml-1 text-sm leading-6">
                            <label for="has_messages" class="font-medium text-surface-fg flex whitespace-nowrap align-text-top">
        ...
        <div class="overflow-hidden">
            <table class="min-w-full divide-y divide-border-soft text-center">
                ...
                <tr class="sticky top-0">
                    ...
                    <th scope="col" class="flex px-6 py-3 text-xs font-medium text-muted 0 uppercase tracking-wider whitespace-nowrap">
                        Call Start &nbsp;
                        <button wire:click="setSorting('Stamp','asc')">
                            <svg  xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-3 @if($sort_by === 'statCallStart.Stamp' && $sort_direction === 'asc') {{ 'text-surface-fg' }} @else {{ 'text-muted' }} @endif">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 6.75 12 3m0 0 3.75 3.75M12 3v18" />
                            </svg>
                        </button>
                        <button wire:click="setSorting('Stamp','desc')">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-3 @if($sort_by === 'statCallStart.Stamp' && $sort_direction === 'desc') {{ 'text-surface-fg' }} @else {{ 'text-muted' }} @endif">
                    ...
                    <th scope="col" class="px-6 py-3 text-xs font-medium text-muted 0 uppercase tracking-wider">
                        Client
                    </th>
                    <th scope="col" class="px-6 py-3 text-xs font-medium text-muted 0 uppercase tracking-wider whitespace-nowrap">
                        Agent(s)
                    ...
                    <th scope="col" class="flex px-6 py-3 text-xs font-medium text-muted 0 uppercase tracking-wider whitespace-nowrap">
                        ...
                        <button wire:click="setSorting('Duration','desc')">
                            <svg  xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-3 @if($sort_by === 'CallDuration' && $sort_direction === 'desc') {{ 'text-surface-fg' }} @else {{ 'text-muted' }} @endif">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 6.75 12 3m0 0 3.75 3.75M12 3v18" />
                            </svg>
                        </button>
                        <button wire:click="setSorting('Duration','asc')">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-3 @if($sort_by === 'CallDuration' && $sort_direction === 'asc') {{ 'text-surface-fg' }} @else {{ 'text-muted' }} @endif">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 17.25 12 21m0 0-3.75-3.75M12 21V3" />
                            </svg>
                        </button>
                ...
                @forelse($call_log as $recent)
                    <tr class="group  transform transition duration-700 ease-in-out">
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-surface-fg transform transition duration-700 ease-in-out">
                            <a class="hover:cursor-pointer font-semibold hover:underline text-primary hover:text-primary transform transition duration-700 ease-in-out" href="/utilities/call-lookup/{{ $recent->CallId }}">
                                {{ $recent->CallId }}
                            </a>
                            <div class="block">
                                <div class="inline-flex whitespace-nowrap my-2 text-xs font-semibold px-2 py-1 border border-border rounded-full">
                        ...
                        <td class="px-6 py-4 text-sm text-surface-fg transform transition duration-700 ease-in-out">
                            @if($recent->CallerANI)
                        ...
                        <td class="px-6 py-4 break-normal text-sm text-surface-fg transform transition duration-700 ease-in-out">
                        ...
                        <td class="px-6 py-4 text-md font-mono font-medium text-surface-fg transform transition duration-700 ease-in-out">
                            <div class="block">
                                {{ Helpers::formatDuration($recent->CallDuration ?? 0) }}
                            </div>
                            <div class="inline-flex my-2 whitespace-nowrap">
                                <svg title="Message(s)" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-4 @if($recent->hasMessages) {{ 'text-primary fill-primary-soft' }}@else {{ 'text-subtle' }} @endif">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                                </svg>
                                <svg title="Recording(s)" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-4 @if($recent->hasRecordings) {{ 'text-primary fill-primary-soft' }}@else {{ 'text-subtle' }} @endif">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.91 11.672a.375.375 0 0 1 0 .656l-5.603 3.113a.375.375 0 0 1-.557-.328V8.887c0-.286.307-.466.557-.327l5.603 3.112Z" />
                                </svg>
                                <svg title="Screen Capture(s)" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-4 @if($recent->hasVideo) {{ 'text-primary fill-primary-soft' }}@else {{ 'text-subtle' }} @endif">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M3.375 19.5h17.25m-17.25 0a1.125 1.125 0 0 1-1.125-1.125M3.375 19.5h1.5C5.496 19.5 6 18.996 6 18.375m-3.75 0V5.625m0 12.75v-1.5c0-.621.504-1.125 1.125-1.125m18.375 2.625V5.625m0 12.75c0 .621-.504 1.125-1.125 1.125m1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125m0 3.75h-1.5A1.125 1.125 0 0 1 18 18.375M20.625 4.5H3.375m17.25 0c.621 0 1.125.504 1.125 1.125M20.625 4.5h-1.5C18.504 4.5 18 5.004 18 5.625m3.75 0v1.5c0 .621-.504 1.125-1.125 1.125M3.375 4.5c-.621 0-1.125.504-1.125 1.125M3.375 4.5h1.5C5.496 4.5 6 5.004 6 5.625m-3.75 0v1.5c0 .621.504 1.125 1.125 1.125m0 0h1.5m-1.5 0c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125m1.5-3.75C5.496 8.25 6 7.746 6 7.125v-1.5M4.875 8.25C5.496 8.25 6 8.754 6 9.375v1.5m0-5.25v5.25m0-5.25C6 5.004 6.504 4.5 7.125 4.5h9.75c.621 0 1.125.504 1.125 1.125m1.125 2.625h1.5m-1.5 0A1.125 1.125 0 0 1 18 7.125v-1.5m1.125 2.625c-.621 0-1.125.504-1.125 1.125v1.5m2.625-2.625c.621 0 1.125.504 1.125 1.125v1.5c0 .621-.504 1.125-1.125 1.125M18 5.625v5.25M7.125 12h9.75m-9.75 0A1.125 1.125 0 0 1 6 10.875M7.125 12C6.504 12 6 12.504 6 13.125m0-2.25C6 11.496 5.496 12 4.875 12M18 10.875c0 .621-.504 1.125-1.125 1.125M18 10.875c0 .621.504 1.125 1.125 1.125m-2.25 0c.621 0 1.125.504 1.125 1.125m-12 5.25v-5.25m0 5.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125m-12 0v-1.5c0-.621-.504-1.125-1.125-1.125M18 18.375v-5.25m0 5.25v-1.5c0-.621.504-1.125 1.125-1.125M18 13.125v1.5c0 .621.504 1.125 1.125 1.125M18 13.125c0-.621.504-1.125 1.125-1.125M6 13.125v1.5c0 .621-.504 1.125-1.125 1.125M6 13.125C6 12.504 5.496 12 4.875 12m-1.5 0h1.5m-1.5 0c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125M19.125 12h1.5m0 0c.621 0 1.125.504 1.125 1.125v1.5c0 .621-.504 1.125-1.125 1.125m-17.25 0h1.5m14.25 0h1.5" />
                                </svg>
