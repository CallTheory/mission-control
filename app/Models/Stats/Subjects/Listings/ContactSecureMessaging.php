...
class ContactSecureMessaging extends Stat
{
    ...
    public function tsql(): string
    {
        return trim(<<<'TSQL'
             select
                dirlistings.listId,
                dirContactSecureMessaging.[Name] as [MethodName],
                dirsubjects.[Name] as [DirectorySubject],
                dirviews.[Name] as [View],
                dirContactSecureMessaging.deviceID as [Result]
            from dirContactSecureMessaging
            left join dirlistings  on dirlistings.listid = dirContactSecureMessaging.listid
            left join dirsubjects on dirlistings.subid = dirsubjects.subid
            left join dirViews on dirviews.subId = dirsubjects.subId
            where deviceID like CONCAT ('%', ?, '%')
            TSQL);
    }
