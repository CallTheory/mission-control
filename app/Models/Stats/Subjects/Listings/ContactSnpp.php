...
class ContactSnpp extends Stat
{
    public function tsql(): string
    {
        return trim(<<<'TSQL'
             select
                dirlistings.listId,
                dirContactSnpp.[Name] as [MethodName],
                dirsubjects.[Name] as [DirectorySubject],
                dirviews.[Name] as [View],
                dirContactSnpp.PagerId as [Result]
            from dirContactSnpp
            left join dirlistings  on dirlistings.listid = dirContactSnpp.listid
