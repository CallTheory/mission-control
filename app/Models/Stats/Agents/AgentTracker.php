...
class AgentTracker extends Stat
{
    ...
    {
        return trim(<<<'TSQL'
           select id, Stamp, agtId, Initials, Name, StationNumber as StationId, StationType, [Type] as [TrackerType], [value] as Reason
           from statAgentTracker where agtId = ? and [Stamp] between ? and ? order by [Stamp] desc;
        TSQL);
    }
