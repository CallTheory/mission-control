...
Returns the smallest possible portion of text between two values within a string.

## Request

**Url**: `/api/utilities/text-between`
...
- `GET /api/utilities/text-between?string=<html><title>catch me if you can</title></html>&start=<title>&end=</title>`

A successful response will return a `200 OK` with a JSON-encoded string representing the text found between `start` and `end`.
...
- `GET /api/utilities/text-between`
